...
# BLOCK_LIST = os.getenv('BLOCK_LIST', '').split(',')
BLOCK_LIST = []
...
def start(message):
    ...
    if username in restricted_users:
        bot.send_message(user_id, "Извините, ваш доступ ограничен.")
        return
...
def handle_message(message):
    user_id = message.from_user.id
    # print(len(user_dialogs[user_id]), user_dialogs[user_id])
    ...
    if len(user_dialogs[user_id]) >= ((n_questions * 2) - 1):
        ...
        chatgpt_response = generate_chatgpt_response(finalize, user_dialogs[user_id])
...
def callback_query(call):
    ...
    if call.data == "continue":
        ...
        Начни с приветственной фразы и задай первый вопрос: “Как зовут тебя и твоего партнера?” и получи на него ответ. Получай ответ от человека, не придумывай сама. Не задавай следующий вопрос, пока не получишь на ответ от клиента на текущий вопрос.
        """
        ...
        bot.send_message(user_id, first_question)
